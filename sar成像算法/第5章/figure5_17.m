...
%  距离时域-方位频域
subplot(221),imagesc(abs(Srf_rd))
xlabel('距离向(采样点)'),ylabel('方位频率(采样点)'),title('(a)RD频谱幅度');
subplot(222),imagesc(angle(Srf_rd))
xlabel('距离向(采样点)'),title('(b)RD频谱相位');
%  距离频域-方位频域                  
subplot(223),imagesc(abs(SrF_2d))      
xlabel('距离频率(采样点)'),ylabel('方位频率(采样点)'),title('(c)二维频谱幅度');
subplot(224),imagesc(angle(SrF_2d))
xlabel('距离频率(采样点)'),title('(d)二维频谱相位');
sgtitle('图5.17 零斜视角、正扫频情况下单个点目标的方位频谱')