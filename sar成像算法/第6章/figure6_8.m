...
xlabel('距离时间(采样点)'),ylabel('方位时间(采样点)'),title('(a)实部（八点插值）')
...
xlabel('距离时间(采样点)'),ylabel('方位时间(采样点)'),title('(a)实部（最近邻域插值）')
...
xlabel('距离时间(采样点)'),ylabel('方位时间(采样点)'),title('(a)实部（四点插值）')
...
xlabel('方位向(采样点)'),ylabel('幅度'),title('(a)8点插值')
subplot(232),plot(Arcm_0),axis([0 Naz,0 1])
xlabel('方位向(采样点)'),ylabel('幅度'),title('(b)最近邻域插值')
...
xlabel('方位向(采样点)'),ylabel('幅度(dB)')
sgtitle('图6.8 RCMC不精确时由调制引入的成对回波')
