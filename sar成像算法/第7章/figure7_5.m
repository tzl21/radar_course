...
subplot(514), plot(abs(s_2_t)), hold on, plot(abs(s_3_t)), hold off, 
axis([0 512, 0 12]), legend('原始信号', '变标后的信号'), ylabel('幅度'), title('(d)压缩后的原始信号和变标信号');
subplot(515), plot(abs(s_2_t)), hold on, plot(abs(s_3_t)), hold off, 
axis([428 480, 0 12]), legend('原始信号', '变标后的信号'), xlabel('时间（采样点）'), ylabel('幅度'), title('(e)(d)中右边目标放大后的形式');
sgtitle('图7.5 线性调频变标引起的点目标移动')